...
[KafkaTopicAttribute(EmployeeEvent.DeleteEmployee)]
public class DeleteEmployeeIntegrator : BaseIntegrator, IKafkaEvent
{
    public DeleteEmployeeIntegrator(IMediator mediator, IKafkaProducerService kafkaProducerService) : base(mediator, kafkaProducerService){}

    public async Task Handler(KafkaMessage meesage, CancellationToken token)
    {
        var entityMessage = meesage.Value.ToDeserializeJSON<RequestDeleteEmployeeDto>();
        var entityMap = MapperConfig.Mapper.Map<DeleteEmployeeCommand>(entityMessage);
