...
internal static class PutEmployeeFaker
{
    private static Guid IdEvent = Guid.NewGuid();

    public static RequestUpdateEmployeeDto GenerateItemFake(this Faker<RequestUpdateEmployeeDto> item)
    {
    ...
    public static ResponseUpdateEmployeeDto GenerateItemFake(this Faker<ResponseUpdateEmployeeDto> item)
    {
        item.RuleFor(x => x.EventId, z => IdEvent.ToString());
